...
import com.example.hotel.HotelAdapter;
            ...
            public void run() {
                try {
                    ...
                    while(true){
                        ...
                        if(read!=null){
                            mHandler.post(new com.example.hotel.Host_page_chatting.msgUpdate(read));//받아온 데이터 화면에 출력 cf)handler:msgUpdate 객체를 message queue에 전달하는 함수
                        }
                    }
                } catch (IOException e) {
                    e.printStackTrace();
